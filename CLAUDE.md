# all-the-tags: instructions for AI assistants
...
This repo publishes filament RFID/NFC decoder plugins for Bespok3d, plus the "All the Tags" collection
that installs the whole family (reader + every decoder + Spoolman tracking) at once. Each decoder is
just the payload decoder: it plugs into the live RFID hub shipped by the RFID Spool Reader and maps one
tag standard into the shared `rfid_data.json`. Every decoder carries Python (a Klipper extra: a pure
decode core plus a thin registration shell); the collection is a bundle manifest with no code of its own.

- `rfid-generic-ndef`: Any plain JSON over NDEF (no `protocol` field)
- `rfid-opentag`: OpenTag3D (queengooborg), binary over NDEF
- `rfid-openprinttag`: OpenPrintTag (prusa3d), CBOR over NDEF
