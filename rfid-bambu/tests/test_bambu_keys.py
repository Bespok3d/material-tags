...
def test_derive_wires_salt_uid_ikm_master():
    # The spec: PRK = HMAC(salt=master, ikm=UID); keys are the expand output split by six.
...
def test_derive_wires_salt_uid_ikm_master_regression():
    # Tests for regression on inverted salt and ikm
...
def test_parse_master_key_accepts_clean_hex():
    parsed = parse_master_key("9a759cf2c4f7caff222cb9769b41bc96")
    assert parsed is not None
    assert len(parsed) == 16


def test_parse_master_key_strips_separators_and_case():
    spaced = parse_master_key(" 9A:75:9C:F2:C4:F7:CA:FF:22:2C:B9:76:9B:41:BC:96 ")
    tight = parse_master_key("9a759cf2c4f7caff222cb9769b41bc96")
    assert spaced == tight
